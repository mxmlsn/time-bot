...
const CITY_KEYS = {
    ...
    // БУЭНОС-АЙРЕС (б, b, запятая, и, ба, ba)
    'б': 'buenos', 
    'b': 'buenos',  
...
// Ищет: цифры + минуты (опц) + пробелы + буква/буквы города
// Negative lookahead (?!...) проверяет что после нет букв (даже через пробел)
const regex = /(\d{1,2})(?:[:\.](\d{2}))?\s*([мmvьпpgзбb,иеeyt]|ба|ba)(?!\s*[a-zа-яёA-ZА-ЯЁ])/i;
